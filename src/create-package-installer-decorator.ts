...
export function createPackageInstallerDecorator<TDeepClient extends DeepClientInstance>(deep: TDeepClient): PackageInstallerDecorator<TDeepClient> {
  const log = debug(`@deep-foundation/capacitor-device:${createPackageInstallerDecorator.name}`);
  ...
  } as PackageInstallerDecorator<TDeepClient>, deep)
  log({decorator})
  return decorator;
}
